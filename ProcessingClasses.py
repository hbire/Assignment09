...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        try:
            ...
            raise Exception('If id is not in list')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        # TODO add code as required
        try:
            new_track = DC.Track(track_info.position, track_info.title, track_info.length)
            cd.add_track(new_track)
        except Exception:
            raise Exception('Position is not an integer')
        pass
